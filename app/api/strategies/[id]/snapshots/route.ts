...
export async function POST(
  ...
  try {
    ...
    // Get last snapshot for P&L calculations
    ...
    // For dual-token LP, we need to calculate liquidity L first
    // Use LP value and a reference price within range to estimate L
    const lpPrice = body.lpPrice || (body.token1Price / body.token2Price)
    
    // Clamp LP price to range for liquidity calculation
    // Use midpoint of range as reference if price is outside
    ...
    try {
      ...
      // Estimate L based on token amounts directly
      ...
      // Rough estimation: L ≈ (token1Amount + token2Amount / price) / range_factor
